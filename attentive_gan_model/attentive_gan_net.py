...
class GenerativeNet(cnn_basenet.CNNBaseModel):
    ...
    def compute_autoencoder_loss(self, input_tensor, label_tensor, name, reuse=False):
        ...
        with tf.variable_scope(name, reuse=reuse):
            ...
            vgg_loss = tf.add(lm_loss, lp_loss, name='vgg_loss')

            # # add ssim_loss to loss
            ssim_max = tf.constant(1.0, tf.float32, name="ssim_max")
            ssim_val = tf.image.ssim(label_tensor,output_list[-1],max_val = 255)
            ssim_loss = tf.subtract(ssim_max,ssim_val[0],name="ssim_loss")
            loss = tf.add(vgg_loss,ssim_loss,name='autoencoder_loss')
